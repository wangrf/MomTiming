momTrade.fun<-function(equity.syms= c("000300.SH", "000905.SH"),
                  cash.sym="003003.OF"
                  ){
  ...
  for(sym in acct.syms){
    if(grepl(sym,dataLS)){
      stop(paste0("Do not find data ",sym))
    }
  }
  
  date.range <- index(get(acct.syms[1]))
  portf <- vector("list",length(acct.syms)+1)
  names(portf) <- c(acct.syms,"summary")
  for(i in 1:length(acct.syms)){
    
    temp <- c("qty","avgPrice","curPrice","realizedPos","unrealizedPos","txn","position","equity","holdratio")
    xx <- matrix(0,nrow=length(date.range),ncol=length(temp))
    colnames(xx) <- temp
    xx<- as.xts(xx,order.by=date.range)
    indexTZ(xx) <- "UTC"
    portf[[i]] <- xx
  ...
  orderbook <- data.frame(date=date.range[1],symbol="",tradeside="buy",qty=0,price=0,txn=0,position=0,holdratio=0)
  portf[["summary"]][tradeStart,c("position","equity","total")] <- c(initEq,0,initEq)
  
  
  
  for(i in (tradeStart+1):length(date.range)){
